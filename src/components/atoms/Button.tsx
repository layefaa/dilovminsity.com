...
const Button = ({label, onClick, type = 'button', active}: IButton) => {
  switch (type) {
    ...
    case 'submit':
      return (
          <button onClick={onClick}
                  className={`${styles.buttonStyle} relative transition-all duration-500 ease-out rounded group overflow-hidden bg-ce-secondary-white hover:bg-ce-tertiary-white text-ce-primary-black inline-flex rounded-[5px] items-center`}>
            <span> {label}</span>
            <svg width="13" height="10" viewBox="0 0 13 10"
